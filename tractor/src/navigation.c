...
double distanceCal(double beglat, double beglon, double endlat, double endlon, double gpslat, double gpslon)
{
	...
	return (func_k * gpslon - gpslat + func_b) /(sqrt((func_k * func_k)+1))*DIS_LAT_LON;
}


double distance(double a1, double b1, double a2, double b2)
{
	// double t = ((a1-a2)*DIS_LAT_LON);
	return sqrt(pow((a1-a2)*DIS_LAT_LON, 2) + pow((b1-b2)*DIS_LAT_LON,2));
}
...
struct CTRL_MESSAGES* navigation(struct GPS_MESSAGES* gps_msg, double endlat[], double endlon[], double endturn[], int* count)
{
  ...
  struct CTRL_MESSAGES * ctrl_msg = (struct CTRL_MESSAGES*)malloc(sizeof(struct CTRL_MESSAGES));
  ctrl_msg->speed = 5;
  ctrl_msg->steer = 0;
  ctrl_msg->power = 1;
  ...
  ctrl_msg->steer = yaw_error*PID_P + 90;
