...
{  
   ...
   if( NULL == (fread = fopen(file_name, "r")))  
   {  
...
{  
	...
	cluster_count += 1;  
	printf("\ncluster_count is %d\n", cluster_count);  
	ps_get_cluster(un_accessed_data, old_unaccessed_data, cluster_count);  
	ps_update_core_object(un_accessed_data);  
	test_counter_1++;  
	}  
	ps_save_noise(un_accessed_data);  
}  
 ...
 * 
 */  
void ps_get_cluster(int* un_accessed_data, int* old_unaccessed_data, int cluster_ID)  
{  
	char file_name[200];  
	sprintf(file_name, "../DBSCAN_cluster/cluster_%d.txt", cluster_ID);  
	FILE* fwrite;  
...
void ps_save_noise(int* un_accessed_data)  
{  
	FILE* fwrite_noise;  
	if( NULL == (fwrite_noise = fopen("../DBSCAN_cluster/noise.txt", "w")))  
	{  
